...
class ItemAdapter (private val mContext: Context, private val itemList: List<SearchItem>, var type: String) :
    RecyclerView.Adapter<ItemAdapter.ItemViewHolder>() {
    private val viewPool = RecycledViewPool()
    private var subItemAdapter: SubItem_Adapter? = null
    ...
    override fun onBindViewHolder(itemViewHolder: ItemViewHolder, i: Int) {
        ...
        val layoutManager = GridLayoutManager(itemViewHolder.rvSubItem.context, 3)
        layoutManager.initialPrefetchItemCount = item.subItemList.size

        // Create sub item view adapter
        subItemAdapter = SubItem_Adapter(mContext,item.subItemList,type)
